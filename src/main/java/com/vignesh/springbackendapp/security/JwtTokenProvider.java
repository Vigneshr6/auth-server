...
public class JwtTokenProvider {
  ...
  @Value("${security.jwt.secret:mysecretkey}") private String secretKey;

  @Value("#{${security.jwt.valid-for: 30} * 1000}") private long validFor;
  ...
  public Authentication getAuthentication(String token) {
    ...
    List<Role> authorities =
        ((List<String>) body.get("auth"))
            .stream().map(a -> Role.valueOf(a)).collect(Collectors.toList());
